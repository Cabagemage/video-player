...
function VideoPlayer({videoUrl, marks}: VideoPlayerProps) {
    ...
    return (
        <div className={clsx(style.wrapper, {[style.videoPlayerFullScreen]: isFullScreen})}>
            <div onMouseLeave={onVideoLeave} onMouseEnter={onVideoHover}  className={clsx(style.videoPlayer, {[style.videoPlayerFullScreen]: isFullScreen})}>
                <video   controls={false} playsInline webkitplaysinline   preload={"auto"}  onDoubleClick={onDoubleClickHandler} onClick={playVideo} style={{width: "100%", height: "100%"}} onLoadedMetadata={handleLoadedMetadata} ref={videoRef} onTimeUpdate={handleTimeUpdate} >
                    Sorry, your browser doesn't support embedded videos.
                </video>
                <ControlPanel isControlPanelVisible={isControlPanelVisible} settings={{quality: {onQualityClick: changeQuality, options: availableQualities, current: availableQualities[currentQuality]?.quality}}} onFullScreenEnter={toggleFullScreen} hoveredTime={hoveredTime} marks={marks} videoDuration={duration} onVideoPlayClick={playVideo} isPlaying={isPlaying} uploadedProgressPosition={uploadedProgressPosition} uploadedProgress={uploadedProgress} onMouseMove={handleMouseMove} onProgressBarClick={handleProgressBarClick} widthOfVideoLength={percent} time={playedTime} ref={progressBarRef} />
            </div>
            <div className={"list"}>
                {marks.map((item,idx) => {
                    const normalizedIndex = idx + 1;
                    return (
                        <button onClick={() => {onListItemClick(item.start)}} key={normalizedIndex} className={"button"}>{`${normalizedIndex}. ${item.text}`}</button>
                    )
                })}
            </div>
        </div>
